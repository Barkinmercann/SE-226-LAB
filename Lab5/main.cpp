...
string passGen(int user_number) {
    ...
    while (user_number < 3 or user_number > 7) {
        cout << "Invalid input. Enter a number between 3 and 7: " << endl;
        cin >> user_number;
    }
    string password = "";
...
string rep_with_upper(string password){
    ...
    int index = rand() % password.length();

    char ch = password[index];
    password[index] = toupper(ch);
...
bool search_letter(string password,char character){
    if(password.find(character)){ return true;}
    else{ return false;}
}
...
int main() {
    int user_number;
    cout << "Enter the number of words for the password (between 3 and 7): ";
    cin >> user_number;

    string password = passGen(user_number);
    cout << "Initial password: " << password << endl;

    string modifiedPassword = rep_with_upper(password);
    ...
    cout << search_letter(finalPassword,'a');
